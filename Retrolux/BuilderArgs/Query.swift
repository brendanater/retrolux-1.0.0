...
//
//        guard let data = components.query?.data(using: encoding) else {
//            throw QueryApplyError.failedToGetHTTPBody(withItems: queryItems)
//        }
//
//        return Body(.data(data), [.contentType: "application/x-www-form-urlencoded" + (encoding.charset.map { "; charset=\($0)" } ?? ""), .contentLength: data.count.description])
//    }
//
//    // strategy
//
